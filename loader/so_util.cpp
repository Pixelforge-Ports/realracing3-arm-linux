...
so_module *so_load_module(const char *filename, struct zip *apk, void *vm) {
    ...
    for (int i = 0; i < modules.size(); i++) {
      ...
      if (so_alt_searchpath && *so_alt_searchpath) {
          ...
          // and so always says "armeabi-v7a" on a 32-bit build; Real Racing 3 is
...
load_module_success:
    ...
    for (auto it = loaded_modules.rbegin(); it != loaded_modules.rend(); it++) {
      ...
      // constructors - 209 of them in Real Racing 3 - and any one of them that
      ...
      // way to opt out. Real Racing 3 needs that: its boot sequence is
 ...
 *
 *     arm-linux-gnueabihf-objdump -d --start-address=0x<off> libRealRacing3.so
 *
...
void so_initialize(so_module *mod) {
  ...
  for (int i = 0; i < mod->num_init_array; i++) {
    ...
    // one. libRealRacing3.so has exactly such a slot: the last entry of its
    // .init_array is a placeholder, and so is the last of its .fini_array.
    //
