 ...
 */
extern "C" int realracing3_open(const char *path, int flags, ...)
{
    mode_t mode = 0;
...
extern "C" int realracing3_ioctl(int fd, int request, ...)
{
    va_list ap;
    va_start(ap, request);
 ...
 * Real Racing 3's does, so leaving it unresolved fails the load outright.
 *
 ...
 */
extern "C" void realracing3_assert2(const char *file, int line,
...
DynLibFunction symtable_libm[] = {
     ...
     * the double return needs the bridge. First importer: Real Racing 3. */
    ...
    THUNK_SPECIFIC("lrintf", HOST_MATH::lrintf),
    THUNK_SPECIFIC("modff",  HOST_MATH::modff),
    ...
    NO_THUNK("open",  (uintptr_t)&realracing3_open),
    NO_THUNK("ioctl", (uintptr_t)&realracing3_ioctl),
    /* No float crosses the boundary, so no ABI bridge is needed. */
    NO_THUNK("__assert2", (uintptr_t)&realracing3_assert2),
