 ...
 * installed, so SDL reported "Can't load EGL/GL library on window creation" and
 * every GL import resolved to nil. That report came from the Dead Space port;
 * this one searches the same directories on the same firmwares and would have
 * committed to the same blob, so the preflight belongs here too.
 *
 ...
 * the subcommand: the launcher runs `realracing3 --gl-probe <library> [symbol
 * ...]` before committing to a candidate, and falls through to the next tier
 * when it fails.
 */
#ifndef REALRACING3_GL_PROBE_H
#define REALRACING3_GL_PROBE_H
