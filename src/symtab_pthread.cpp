 ...
 * sometimes. REALRACING3_LATE_THREAD_MS exists so the next person can find out
...
static bool is_late_thread(void *(*entry)(void *))
{
    ...
    const char *want = getenv("REALRACING3_LATE_THREAD_AT");
    if (!want || !*want)
...
{
    const char *env = getenv("REALRACING3_LATE_THREAD_MS");
    /*
    ...
    trace("  ^ delayed %d ms before starting (engine startup race; "
          "REALRACING3_LATE_THREAD_MS=0 disables)", ms);
 ...
 *
 * That is where Real Racing 3 died, before onCreate and before anything was logged:
 ...
 *
 * (librealracing3.so ships it at .data+0x1210c0, word = 0x4000), and the engine's
 ...
 * still showing nothing but "Linking librealracing3.so...".
 *
...
int bionic_pthread_mutex_init(BIONIC_pthread_mutex_t *m, pthread_mutexattr_t **attr)
{
     ...
     *
     * Real Racing 3 allocates its mutexes with the engine's own allocator, which
     ...
     * word the game reserved. librealracing3.so imports no pthread_mutexattr_*
...
/*
 * How often the guest takes a mutex, counted because nobody ever had.
 *
 * EAThread emulates 64-bit atomics with a table of 32 mutexes hashed by
 * address (see the long note above), locked on every 64-bit load and store from
 * every thread. Each one of those arrives here. If the game does that a
 * hundred thousand times a frame it costs a few milliseconds; if it does it two
 * million times, it is the frame - and this is the only path through the loader
 * that is dimensionally capable of explaining tens of milliseconds.
 *
 * Relaxed on purpose: the value is only read for a report, and a seq_cst
 * counter on the hottest function in the loader would be measuring itself.
 */
static std::atomic<unsigned long> g_mutex_locks(0);

unsigned long bionic_mutex_lock_count(void)
{
    return g_mutex_locks.load(std::memory_order_relaxed);
}

int bionic_pthread_mutex_lock(BIONIC_pthread_mutex_t *m)
{
    g_mutex_locks.fetch_add(1, std::memory_order_relaxed);
