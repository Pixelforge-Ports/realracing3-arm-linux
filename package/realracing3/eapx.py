...
class PortMaster:
    ...
    def progress(self, text, done, total, force=False):
        now = time.time()
...
class Progress:
    ...
    def update(self, state=1, overall=0, message="", detail="", force=False):
        ...
        self.last_write = now
        self._render(*record)
        if self.portmaster is not None:
            if message and message != self.spoken:
                self.spoken = message
                if under_patcher():
                    sys.stdout.write(message.capitalize() + "\n")
                    sys.stdout.flush()
                self.portmaster.send("message", message.capitalize())
            percent = max(0, min(100, int(overall) // 10))
            text = "%s - %d%%" % (message.capitalize(), percent)
            if self.total_bytes:
                text += " (%s / %s)" % (human_bytes(self.done_bytes), human_bytes(self.total_bytes))
            if state == 2:
                self.portmaster.send("message", "Setup failed: " + message)
            else:
                self.portmaster.progress(text, max(0, min(1000, int(overall))), 1000,
                                         force=force or state == 3)
        if not self.path:
            return
