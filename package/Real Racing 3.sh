...
export REALRACING3_RESOLUTION="${REALRACING3_RESOLUTION:-auto}"
if [ -f "$GAMEDIR/resolution.txt" ]; then
  export REALRACING3_RESOLUTION="$(tr -d '\r\n' < "$GAMEDIR/resolution.txt")"
fi
...
EOF
  ...
  if command -v pm_message >/dev/null 2>&1; then
    pm_message "Real Racing 3: preparing game data. Keep the device powered on."
...
mapper_pid=
if [ -n "${GPTOKEYB2:-}" ]; then
  $GPTOKEYB2 "realracing3" -c "$GAMEDIR/realracing3.ini" &
  mapper_pid=$!
fi
