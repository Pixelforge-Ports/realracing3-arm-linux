...
extern DynLibFunction symtable_gl_stubs[];
...
__eglMustCastToProperFunctionPointerType eglGetProcAddress(const char *procname)
{
    ...
    /* Optional GLES3/vendor calls are deliberately provided as safe no-ops
     * by symtab_gl_stubs. Runtime lookups must see the same compatibility
     * functions as ELF imports; otherwise RR3 stores NULL and jumps to it in
     * mtRenderGL::discardFramebuffer(). */
    for (int i = 0; symtable_gl_stubs[i].symbol; i++) {
        if (strcmp(symtable_gl_stubs[i].symbol, procname) == 0)
            return (__eglMustCastToProperFunctionPointerType)symtable_gl_stubs[i].func;
    }
...
/*
 * The table the loader binds into librealracing3.so for its libEGL.so imports.
 */
#include "thunk_gen.h"
