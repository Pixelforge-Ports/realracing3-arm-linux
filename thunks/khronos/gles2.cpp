...
/* Mesa's ES2 context omits several optional OES/EXT entry points that RR3
 * calls unconditionally on its Android target.  Keep those calls harmless. */
static void rr3_discard(GLenum, GLsizei, const GLenum *) {}
static void rr3_bind_vao(GLuint) {}
static void rr3_gen_vao(GLsizei n, GLuint *a) { for (GLsizei i=0; i<n && a; ++i) a[i]=i+1; }
static void rr3_delete_vao(GLsizei, const GLuint *) {}
static GLboolean rr3_is_vao(GLuint) { return GL_FALSE; }
static void *rr3_map_buffer(GLenum, GLenum) { return nullptr; }
static GLboolean rr3_unmap_buffer(GLenum) { return GL_FALSE; }
...
/*
 * Make an EXT_shader_framebuffer_fetch shader compile without the extension,
 * WITHOUT changing which variant the engine thinks it compiled.
 *
 * Measured, not assumed. Every Shaders/full_screen_effects compile arrives with
 * this prologue synthesised by the engine (dumped with
 * REALRACING3_SHADER_DUMP_DIR, 16 programs, ids 6..51):
 *
 *     #ifndef COLOR_CURVES
 *     #define COLOR_CURVES
 *     #endif
 ...
 *     #endif
 *
 * The previous rewrite deleted that `#define FRAMEBUFFER_FETCH`, on the theory
 * that the shader would then take its sampler2D branch. It does - and that is
 * the bug this cost us. The engine picks the variant BEFORE it hands us the
 * source, and it sets the draw up to match:
 *
 *   - full_screen_effects.vsh has two mutually exclusive bodies. The fetch one
 *     declares only a_Position; the sampler one also declares a_TexCoord0 and
 *     the varying v_TexCoord. The engine bound vertex attributes for the fetch
 *     body, so switching bodies leaves v_TexCoord undefined.
 *   - full_screen_effects.fsh reads the destination through FRAMEBUFFER_RGBA.
 *     The sampler branch reads texture2D(s_Texture0, v_TexCoord); the engine
 *     never bound a screen texture to s_Texture0, because in the variant it
 *     chose the colour comes from gl_LastFragData[0].
 *
 * The result compiles, links and reports no GL error, and draws an OPAQUE
 * full-screen quad of one flat colour over the finished 3D scene - which is
 * exactly what the census caught at the end of every race frame (bucket
 * prog=51 fbo=0 TRIANGLE_STRIP depth=0 blend=0 count=4, after 2.8 M vertices
 * of world geometry had already landed in the same framebuffer).
 *
 * So keep the variant and remove only the extension. FRAMEBUFFER_RGBA becomes a
 * function that discards, which leaves the destination untouched: the colour
 * grading pass turns into a no-op instead of a paint bucket. The grading itself
 * (brightness/contrast) is lost until it is reimplemented from a copy of the
 * backbuffer; the scene underneath is not.
 *
 * Not needed on the R36S - the Mali-G31 exposes GL_EXT_shader_framebuffer_fetch
 * and runs the original path - so this stays behind
 * REALRACING3_DISABLE_FRAMEBUFFER_FETCH, which only the emulator sets.
 */
static void rr3_neutralise_framebuffer_fetch(std::string &source)
{
    /* `require` is fatal on a driver that lacks the extension, so it goes
     * first. Both patterns live only in .fsh sources, which is why the vertex
     * shader comes through byte for byte and keeps the attribute set the
     * engine bound. The helper costs one extra line, so compile-error line
     * numbers shift by one past it; nothing else moves. */
    const struct { const char *from; const char *to; } patches[] = {
        { "#extension GL_EXT_shader_framebuffer_fetch : require",
          "// GL_EXT_shader_framebuffer_fetch unavailable on this driver" },
        { "#define FRAMEBUFFER_RGBA gl_LastFragData[0]",
          "lowp vec4 rr3_fbfetch() { discard; return vec4(0.0); }\n"
          "#define FRAMEBUFFER_RGBA rr3_fbfetch()" },
    };

    for (const auto &patch : patches) {
        const std::string from = patch.from;
        const std::string to = patch.to;
        size_t at = 0;
...
void glShaderSource_dump(
	...
	const bool disable_fb_fetch = getenv("REALRACING3_DISABLE_FRAMEBUFFER_FETCH") != nullptr;

	/* How much source the engine handed us, before this function touches it.
	 * A shader that fails with "unexpected end of file" is either short on the
	 * way in - the engine's own file read came up short - or short on the way
	 * out, because this function lost part of it. Printing both ends is what
	 * tells those two apart, and the sum below is the "way in" end. */
	if (gl_stats_enabled()) {
		size_t incoming = 0;
		for (GLsizei i = 0; i < count; i++) {
			if (lengths && lengths[i] >= 0)
				incoming += (size_t)lengths[i];
			else if (strings && strings[i])
				incoming += strlen(strings[i]);
		}
		fprintf(stderr,
			"GLSTATS: shaderSource id=%u count=%d incoming=%zu bytes "
			"first-length=%d\n",
			shader, (int)count, incoming,
			lengths ? (int)lengths[0] : -1);
		fflush(stderr);
	}

	// Keep the fast path when neither shader overrides nor compatibility
	// rewriting is requested.
	if (shader_override_dir.empty() && !disable_fb_fetch) {
    ...
    if (disable_fb_fetch)
        rr3_neutralise_framebuffer_fetch(source);

    if (shader_override_dir.empty()) {
        const GLchar *ptr = source.c_str();
        GLint len = (GLint)source.size();
        if (gl_stats_enabled()) {
            fprintf(stderr, "GLSTATS: shaderSource id=%u outgoing=%d bytes\n",
                    shader, (int)len);
            fflush(stderr);
        }
...
void load_gles2_funcs()
{
	...
	glad_glGetString = (PFNGLGETSTRINGPROC)PTR_RESOLVE(glGetString);
	/* resolve_thunked() returns a wrapper even when SDL cannot resolve the
	 * optional entry point; calling that wrapper would jump through NULL.
	 * RR3 only uses discard as a bandwidth hint, so make it harmless on
	 * drivers without GL_EXT_discard_framebuffer. */
	glad_glDiscardFramebufferEXT = (PFNGLDISCARDFRAMEBUFFEREXTPROC)rr3_discard;
	if (!glad_glBindVertexArrayOES) glad_glBindVertexArrayOES = (PFNGLBINDVERTEXARRAYOESPROC)rr3_bind_vao;
	if (!glad_glGenVertexArraysOES) glad_glGenVertexArraysOES = (PFNGLGENVERTEXARRAYSOESPROC)rr3_gen_vao;
	if (!glad_glDeleteVertexArraysOES) glad_glDeleteVertexArraysOES = (PFNGLDELETEVERTEXARRAYSOESPROC)rr3_delete_vao;
	if (!glad_glIsVertexArrayOES) glad_glIsVertexArrayOES = (PFNGLISVERTEXARRAYOESPROC)rr3_is_vao;
	if (!glad_glMapBufferOES) glad_glMapBufferOES = (PFNGLMAPBUFFEROESPROC)rr3_map_buffer;
	if (!glad_glUnmapBufferOES) glad_glUnmapBufferOES = (PFNGLUNMAPBUFFEROESPROC)rr3_unmap_buffer;
}
