#!/usr/bin/env python3
"""MCP server for the Real Racing 3 qemu/Mesa development emulator.
...
RUNTIME_DIR = Path(
    os.environ.get("REALRACING3_EMULATOR_RUNTIME", PORT_DIR / "emulator/runtime")
).resolve()
...
def start_emulator(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["REALRACING3_CONTROL_DIR"] = str(RUNTIME_DIR)
    if _time_scale > 1.0:
        env["REALRACING3_TIME_SCALE"] = f"{_time_scale:g}"
    if (PORT_DIR / "build/realracing3").exists() and not arguments.get("rebuild"):
        env["REALRACING3_EMULATOR_SKIP_BUILD"] = "1"
...
def hold_control(arguments: dict[str, Any]) -> dict[str, Any]:
    """Half of press_control: one edge only, so a control can stay held.

    Racing needs held controls, not taps. The throttle is an axis the engine
    samples once per frame (fmJoystickAndroid::SetAxisValues stores it and
    JoystickInput::update reads the stored value), so "hold R2" means sending
    the down edge and not sending the up edge until we want to lift.
    """
    control = str(arguments["control"]).lower()
    if control not in VALID_CONTROLS:
        raise ValueError(f"unknown control {control!r}")
    down = bool(arguments.get("down", True))
    _append(f"button {control} {'down' if down else 'up'}")
    return _text(f"{control} {'held' if down else 'released'}.")
...
TOOLS: list[dict[str, Any]] = [
    {
        "name": "start_emulator",
        "description": "Start the persistent qemu-arm/Mesa Real Racing 3 emulator and wait until its control channel is ready.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "rebuild": {
    ...
    {
        "name": "hold_control",
        "description": "Hold or release one R36S control (one edge only), for throttle and brake.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "control": {"type": "string", "enum": sorted(VALID_CONTROLS)},
...
def _handle(message: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if method == "initialize":
        ...
        result = {
            ...
            "serverInfo": {"name": "realracing3-emulator", "version": "0.1.0"},
        }
    elif method == "ping":
        result = {}
